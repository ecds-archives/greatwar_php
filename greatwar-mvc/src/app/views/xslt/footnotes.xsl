...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0"
  ...
  exclude-result-prefixes="html exist">
...
<xsl:param name="next-note-list">|caption|head|q|quote|</xsl:param>
...
<xsl:param name="use-popups">true</xsl:param>
  ...
  <xsl:param name="overlib-url"><xsl:value-of select="$base"/>js/overlib.js</xsl:param>	<!-- path to overlib -->
  <xsl:param name="overlib-img"><xsl:value-of select="$base"/>images/overLib.gif</xsl:param>	<!-- path to overlib icon -->
  ...
  <!-- display page numbers in endnote listing? -->
  <xsl:param name="display-pagenumbers">false</xsl:param>
...
<xsl:template name="next-note">
  ...
  <xsl:variable name="next-text">
    <xsl:value-of select="normalize-space(following::text())"/>
  </xsl:variable>

  <!-- if the note/ref is immediately following, the following text should be that of the note/ref; 
       otherwise there is intervening text & note/ref is not immediately following -->
  <xsl:choose>
    <xsl:when test="$next-name = 'note' and $next-text = $next">
      <xsl:apply-templates select="following::*[1][name() = 'note']">
        <xsl:with-param name="mode">next-note</xsl:with-param>
      </xsl:apply-templates>
    </xsl:when>
    <xsl:when test="$next-name = 'ref' and $next-text = $next">
...
<xsl:template match="ref">
  ...
  <!-- pick up previous text and previous sibling to compare, in order to correctly detect next-note conditions -->
  <xsl:variable name="prev-text"><xsl:value-of select="preceding::text()[1]"/></xsl:variable>
  <xsl:variable name="prev-sib-text"><xsl:value-of select="preceding-sibling::*[1]"/></xsl:variable>



  <xsl:if test="($ref-mode = 'ref' and ($prev = ''))
                or (not(contains($next-note-list, $prev-match)))
                or (contains($next-note-list, $prev-match) and $prev-text != $prev-sib-text)
                or $mode = 'next-note'">
    <a> 
...
<xsl:if test="count(//note[not(ancestor::teiHeader)]) > 0">
    <xsl:element name="div">
      <xsl:attribute name="class">endnote</xsl:attribute>
      <xsl:element name="h2">Notes</xsl:element>
      <xsl:apply-templates select="//note[not(parent::note) and not(ancestor::teiHeader)]" mode="end"/>
    </xsl:element>
   ...
   <xsl:if test="$use-popups = 'true'">
     ...
     <xsl:element name="div">
       ...
       <a href="http://www.bosrup.com/web/overlib/">
         <img id="overlib-img" width="88" height="31" alt="Popups by overLIB" border="0">
           <xsl:attribute name="src"><xsl:value-of select="$overlib-img"/></xsl:attribute>
         </img>
         </a> 
...
<xsl:template match="note" mode="end">
 ...
 needs a little extra logic to ensure the first page # displayed is correct. --> 
  ...
  <xsl:element name="div">
    ...
    <!-- only display page numbers in endnotes if it is enabled -->
  <xsl:if test="$display-pagenumbers = 'true'">
...
<xsl:template match="note/ref" mode="javascript">
  <xsl:text> </xsl:text><xsl:element name="a">
    <xsl:attribute name="href">$self<xsl:value-of select="@target"/></xsl:attribute>
    <xsl:attribute name="target">_top</xsl:attribute>
    <xsl:apply-templates/>
  </xsl:element>
</xsl:template>
...
<xsl:template match="hi" mode="endnote">
 <span>
  ...
  <xsl:template match="note[@id]" mode="javascript">
  ...
  <!-- replace double dash with single (so javascript can be commented out) -->
  <xsl:variable name="doubledash">--</xsl:variable>
  <xsl:variable name="singledash">-</xsl:variable>	<!-- use emdash character? -->
  <xsl:variable name="step2">
    <xsl:call-template name="replace-string">
      <xsl:with-param name="string" select="$step1"/>	<!-- use modified version from step3 -->
      <xsl:with-param name="from" select="$doubledash"/>
      <xsl:with-param name="to" select="$singledash"/>
    </xsl:call-template>
  </xsl:variable>
  ...
  <!-- ignore any notes without ids -->
  <xsl:template match="note[not(@id)]" mode="javascript"/>
...
<!-- do whatever would be normally done with an exist match for highlighting purposes -->
  <xsl:template match="exist:match" mode="javascript">
    <xsl:apply-templates select="."/>
  </xsl:template>

  <xsl:template match="exist:match" mode="endnote">
    <xsl:apply-templates select="."/>
  </xsl:template>
